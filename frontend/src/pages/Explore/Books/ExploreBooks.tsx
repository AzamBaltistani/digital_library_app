...
import { fetchAllContents, fetchBorrowableContents, fetchFreeContents, fetchMostBorrowed, fetchMostDownloaded, type ContentBook } from "@/utils/api_explore_books";

export default function ExploreBooks() {
    const [selectedItem, setSelectedItem] = useState("All");
    const [data, setData] = useState<ContentBook[]>([]);
                            ...
                            {data.length >= 1 ? data.map((item) => (
                                ...
                                >
                                    ...
                                    <div className="mt-1 text-xs">
                                        {
                                            item.book_details?.publication_year ? (<>
                                                Publication Year: {item.book_details?.publication_year}
                                            </>) : (
                                                <> {" "} </>
                                            )
                                        }
                                        {
                                            item.book_details?.volume_number ? (<>
                                                {" | "} Volume: {item.book_details?.volume_number}
                                            </>) : (
                                                <> {" "} </>
                                            )
                                        }
                                ...
                                <div> <p className="text-xl font-semibold ">No data found</p> </div>
                            )}
                        </div>
                    )}
                </div>
